define(["require", "exports"], function (require, exports) {
    // SVG to base64 Image
    function svg2Base64(svgNode, options, cb) {
    ...
    function svgAsDataUri(el, options, cb) {
        ...
        inlineImages(el, function () {
            ...
            var svgNode = doctype + outer.innerHTML;
            var uri = 'data:image/svg+xml;base64,' + window.btoa(reEncode(svgNode));
            if (cb) {
                cb(uri);
            }
        });
            ...
            if (rules != null) {
                for (var j = 0; j < rules.length; j++) {
                    ...
                    if (typeof (rule.style) != "undefined") {
                        var match = void 0, selectorText = void 0;
                        try {
    ...
    function reEncode(data) {
        ...
        data = data.replace(/%([0-9A-F]{2})/g, function (match, p1) {
            var c = String.fromCharCode(Number('0x' + p1));
            return c === '%' ? '%25' : c;
        });
