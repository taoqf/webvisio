define(["require", "exports"], function (require, exports) {
    ...
    function flatten(elem, toCubics, toAbsolute, rectAsArgs, dec) {
        ...
        if (!path_elem || path_elem.getAttribute(d) == '')
            return 'M 0 0';
        ...
        if (!toCubics) {
            ...
            arr = this.path2curve(d); // mahvstcsqz -> MC
            var arr_orig = arr;
        }
        ...
        var i = 0, j, m = arr.length, letter = '', letter_orig = '', x = 0, y = 0, point, newcoords = [], newcoords_orig = [], pt = svgDOM.createSVGPoint(), subpath_start = { x: null, y: null }, prevX = 0, prevY = 0;
        ...
        for (; i < m; i++) {
            ...
            if (letter == 'A') {
                ...
                pt.x = arr[i][6];
                pt.y = arr[i][7];
                newcoords[i] = this.arc_transform(arr[i][1], arr[i][2], arr[i][3], arr[i][4], arr[i][5], pt, matrix);
        ...
        return convertToString(newcoords);
    ...
    function convertToPath(oldElem, rectAsArgs) {
        ...
        var namespaceURI = oldElem.ownerSVGElement ? oldElem.ownerSVGElement.namespaceURI : oldElem.namespaceURI;
        ...
        switch (tag) {
            case 'ellipse':
            case 'circle':
                var rx = +oldElem.getAttribute('rx'), ry = +oldElem.getAttribute('ry'), cx = +oldElem.getAttribute('cx'), cy = +oldElem.getAttribute('cy');
                if (tag == 'circle') {
            ...
            case 'path':
                ...
                break;
            case 'line':
                var x1 = oldElem.getAttribute('x1'), y1 = oldElem.getAttribute('y1');
                var x2 = oldElem.getAttribute('x2');
                var y2 = oldElem.getAttribute('y2');
                d = 'M' + x1 + ',' + y1 + 'L' + x2 + ',' + y2;
                break;
            case 'polyline':
            ...
            case 'polygon':
                ...
                break;
            case 'rect':
                var rx = +oldElem.getAttribute('rx'), ry = +oldElem.getAttribute('ry'), b = oldElem.getBBox(), x = b.x, y = b.y, w = b.width, h = b.height;
    ...
    function arc_transform(a_rh, a_rv, a_offsetrot, large_arc_flag, sweep_flag, endpoint, matrix, svgDOM) {
        ...
        rot = a_offsetrot;
        s = parseFloat(Math.sin(rot).toString());
        c = parseFloat(Math.cos(rot).toString());
    ...
    var p2s = /,?([achlmqrstvxz]),?/gi;
    var split = "split";
    function interPathHelper(path1, path2) {
        path1 = _path2curve(path1);
        path2 = _path2curve(path2);
        var x1, y1, x2, y2, x1m, y1m, x2m, y2m, bez1, bez2, res = [];
    ...
    var _path2curve = function (path, path2) {
    ...
    var _pathToAbsolute = function (pathArray) {
    ...
    var catmullRom2bezier = function (crp, z) {
        var d = [];
        for (var i = 0, iLen = crp.length; iLen - 2 * Number(!z) > i; i += 2) {
            var p = [
                { x: +crp[i - 2], y: +crp[i - 1] },
                { x: +crp[i], y: +crp[i + 1] },
                { x: +crp[i + 2], y: +crp[i + 3] },
    ...
    function interHelper(bez1, bez2) {
        ...
        var l1 = bezlen.apply(0, bez1), l2 = bezlen.apply(0, bez2), n1 = mmax(~~(l1 / 5), 1), n2 = mmax(~~(l2 / 5), 1), dots1 = [], dots2 = [], xy = {}, res = [];
    ...
    var curveDim = function (p1x, p1y, c1x, c1y, c2x, c2y, p2x, p2y) {
